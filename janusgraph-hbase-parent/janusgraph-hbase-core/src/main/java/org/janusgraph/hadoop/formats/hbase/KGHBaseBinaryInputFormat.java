...
public class KGHBaseBinaryInputFormat extends AbstractBinaryInputFormat {

    private static final Logger log = LoggerFactory.getLogger(KGHBaseBinaryInputFormat.class);
    ...
    public void setConf(final Configuration config) {
        ...
        config.set(TableInputFormat.INPUT_TABLE, config.get(STORAGE_HBASE_TABLE));
        String zoomkeeperUri = config.get(STORAGE_HOSTNAME);
        config.setStrings(HConstants.ZOOKEEPER_QUORUM, zoomkeeperUri.split(","));
        if (config.onlyKeyExists(STORAGE_PORT))
            config.set(HConstants.ZOOKEEPER_CLIENT_PORT, String.valueOf(config.get(STORAGE_PORT)));
