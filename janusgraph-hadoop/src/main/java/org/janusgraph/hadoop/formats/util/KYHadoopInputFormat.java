...
public abstract class KYHadoopInputFormat extends InputFormat<NullWritable, VertexWritable> implements Configurable, GraphFilterAware {

    private final InputFormat<StaticBuffer, PropertyEntry> inputFormat;
    ...
    public KYHadoopInputFormat(InputFormat<StaticBuffer, PropertyEntry> inputFormat) {
    ...
    public RecordReader<NullWritable, VertexWritable> createRecordReader(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
        return new KYHadoopRecordReader(refCounter, inputFormat.createRecordReader(split, context));
    }
