...
public class InputRDDTest extends AbstractSparkTest {
    ...
    public void shouldReadFromArbitraryRDD() {
        ...
        assertEquals(123l, graph.traversal().withComputer(SparkJanusGraphComputer.class).V().values("age").sum().next());
        assertEquals(Long.valueOf(4l), graph.traversal().withComputer(SparkJanusGraphComputer.class).V().count().next());
    }

    @Test
    public void shouldSupportJanusGraphOLTP() {
        final Configuration configuration = new BaseConfiguration();
        configuration.setProperty("spark.master", "local[4]");
        configuration.setProperty("spark.serializer", GryoSerializer.class.getCanonicalName());
        configuration.setProperty(Graph.GRAPH, JanusGraph.class.getName());
        configuration.setProperty(Constants.GREMLIN_HADOOP_GRAPH_READER, ExampleInputRDD.class.getCanonicalName());
        configuration.setProperty(Constants.GREMLIN_HADOOP_GRAPH_WRITER, GryoOutputFormat.class.getCanonicalName());
        configuration.setProperty(Constants.GREMLIN_HADOOP_OUTPUT_LOCATION, TestHelper.makeTestDataDirectory(this.getClass(), "shouldSupportJanusGraphOLTP"));
        configuration.setProperty(Constants.GREMLIN_HADOOP_JARS_IN_DISTRIBUTED_CACHE, false);
        ////////
        Graph graph = GraphFactory.open(configuration);
