...
public class GryoSerializerIntegrateTest extends AbstractSparkTest {
    ...
    public void shouldHaveAllRegisteredGryoSerializerClasses() throws Exception {
        ...
        final GraphTraversal.Admin<Vertex, Map<Vertex, Collection<Vertex>>> traversal = graph.traversal().withComputer(SparkJanusGraphComputer.class).V().group("m").<Map<Vertex, Collection<Vertex>>>cap("m").asAdmin();
        ...
        assertEquals(totalVertices, graph.compute(SparkJanusGraphComputer.class).program(PageRankVertexProgram.build().iterations(2).create(graph)).submit().get().graph().traversal().V().count().next().longValue());
        ...
        configuration.setProperty(Constants.GREMLIN_SPARK_PERSIST_CONTEXT, true);
        graph = GraphFactory.open(configuration);
        assertEquals(totalVertices, graph.traversal().withComputer(SparkJanusGraphComputer.class).V().count().next().longValue());
