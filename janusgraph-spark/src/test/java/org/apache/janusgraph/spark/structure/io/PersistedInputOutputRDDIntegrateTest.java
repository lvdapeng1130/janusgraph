...
public class PersistedInputOutputRDDIntegrateTest extends AbstractSparkTest {
    ...
    public void shouldNotHaveDanglingPersistedComputeRDDs() throws Exception {
        ...
        ///
        assertEquals(6, graph.traversal().withComputer(Computer.compute(SparkJanusGraphComputer.class)).V().out().count().next().longValue());
        assertFalse(Spark.hasRDD(Constants.getGraphLocation(rddName)));
        assertEquals(0, Spark.getContext().getPersistentRDDs().size());
        //
        assertEquals(2, graph.traversal().withComputer(Computer.compute(SparkJanusGraphComputer.class)).V().out().out().count().next().longValue());
        assertFalse(Spark.hasRDD(Constants.getGraphLocation(rddName)));
        assertEquals(0, Spark.getContext().getPersistentRDDs().size());
        ///////
        Spark.close();
        ...
        for (final String storageLevel : Arrays.asList("MEMORY_ONLY", "DISK_ONLY", "MEMORY_ONLY_SER", "MEMORY_AND_DISK_SER")) {
            ...
            graph.compute(SparkJanusGraphComputer.class)
                    ...
                    .program(TraversalVertexProgram.build()
                            .traversal(graph.traversal().withComputer(SparkJanusGraphComputer.class),
                                    "gremlin-groovy",
    ...
    public void shouldPersistRDDAcrossJobs() throws Exception {
        ...
        assertEquals(6, graph.traversal().withComputer(SparkJanusGraphComputer.class).V().out().count().next().longValue());
        assertTrue(Spark.hasRDD(Constants.getGraphLocation(rddName)));
        assertEquals(1, Spark.getContext().getPersistentRDDs().size());
        ///////
        graph = GraphFactory.open(configuration);
    ...
    public void testComplexChain() throws Exception {
        ...
        assertEquals(0, Spark.getContext().getPersistentRDDs().size());
        Graph graph = GraphFactory.open(configuration);
        graph = graph.compute(SparkJanusGraphComputer.class).persist(GraphComputer.Persist.EDGES).program(PageRankVertexProgram.build().iterations(2).create(graph)).submit().get().graph();
        ...
        ////
        graph = GraphFactory.open(configuration);
        graph = graph.compute(SparkJanusGraphComputer.class).persist(GraphComputer.Persist.EDGES).mapReduce(PageRankMapReduce.build().create()).program(PageRankVertexProgram.build().iterations(2).create(graph)).submit().get().graph();
        g = graph.traversal();
        assertEquals(6l, g.V().count().next().longValue());
        assertEquals(6l, g.E().count().next().longValue());
        assertEquals(6l, g.V().values(PageRankVertexProgram.PAGE_RANK).count().next().longValue());
        ...
        ////
        graph = GraphFactory.open(configuration);
        graph = graph.compute(SparkJanusGraphComputer.class).persist(GraphComputer.Persist.VERTEX_PROPERTIES).program(PageRankVertexProgram.build().iterations(2).create(graph)).submit().get().graph();
        g = graph.traversal();
        assertEquals(6l, g.V().count().next().longValue());
        ...
        ////
        graph = GraphFactory.open(configuration);
        graph = graph.compute(SparkJanusGraphComputer.class).persist(GraphComputer.Persist.NOTHING).program(PageRankVertexProgram.build().iterations(2).create(graph)).submit().get().graph();
